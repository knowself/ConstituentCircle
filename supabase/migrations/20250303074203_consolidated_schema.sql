-- Migration: 20250303074203_consolidated_schema.sql
-- Consolidated Schema for Constituent Circle Database
-- This file combines all migrations into a single file for database reinitialization
-- Created on: 2025-03-03 07:42:03 UTC
...
-- Using DO block to avoid errors if RLS is already enabled
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        -- Do nothing, RLS might already be enabled
END $$;

-- Add indexes for better query performance
-- Using DO block to avoid errors if indexes already exist
DO $$
BEGIN
    -- Create indexes if they don't exist
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_representatives_email') THEN
        CREATE INDEX idx_representatives_email ON representatives(email);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_constituents_email') THEN
...
EXCEPTION
    WHEN OTHERS THEN
        -- Do nothing, indexes might already exist
END $$;

-- Create RLS policies
-- Using DO block to handle policy creation and avoid errors if policies already exist
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        -- Do nothing, policies might already exist or have different names
END $$;
